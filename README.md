...
- Review all weeks of the current calendar year in a chart and breakdown
...
Weekly summaries use Monday-to-Sunday periods. The first and last weeks are clipped to the current calendar year, so every date in that year is represented exactly once. Weekly calculations always use the complete expense collection and are not affected by description search.
...
- The weekly summary shows the current calendar year.
...
- The summary year is fixed to the current year.
